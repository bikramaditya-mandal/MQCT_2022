...
c-------------------------------- PES EVALUATION
     ...
     $        mxlam, nlam, lam, R_d, coef,ityp)
         ...
         do i=1,nlam
            ...
            p2=lam(3,i)
            p =lam(4,i)
            y=y+t(p1,q1,p2,p,thx,phx,thppx,phppx)*coef(i)
         enddo
      V = y
...
c
   ...
   20 return
		...
		subroutine xyz(r_w_h,a1,b1,g1,a2,b2,g2,theta,phi,thetap,phip)
		implicit none
		...
		real*8 a2,b2,g2,rot_mat2(3,3),temp_rot_mat1(3,3)
...
!		print*,'theta=',theta*180d0/pi
!		print*,'phi=',phi*180d0/pi
...
!		print*,'theta_prime=',thetap*180d0/pi
!		print*,'phi_prime=',phip*180d0/pi
		
		end subroutine

		subroutine rot_mat(a1,b1,g1,mat)
		implicit none
		real*8 a1,b1,g1,mat(3,3)	!angles are supplied in radian
		
		mat(1,1)=-dsin(g1)*dsin(a1)+dcos(b1)*dcos(g1)*dcos(a1)
		mat(1,2)=dsin(a1)*dcos(g1)+dcos(b1)*dsin(g1)*dcos(a1)
		mat(1,3)=-dcos(a1)*dsin(b1)
		
		mat(2,1)=-dcos(a1)*dsin(g1)-dcos(b1)*dcos(g1)*dsin(a1)
		mat(2,2)=dcos(a1)*dcos(g1)-dcos(b1)*dsin(g1)*dsin(a1)
		mat(2,3)=dsin(a1)*dsin(b1)
		
		mat(3,1)=dsin(b1)*dcos(g1)
		mat(3,2)=dsin(b1)*dsin(g1)
		mat(3,3)=dcos(b1)
		endsubroutine
		
		subroutine inv_rot_mat(a1,b1,g1,mat)
		implicit none
		real*8 a1,b1,g1,mat(3,3)	!angles are supplied in radian
		
		mat(1,1)=-dsin(a1)*dsin(g1)+dcos(b1)*dcos(g1)*dcos(a1)
		mat(2,1)=dsin(a1)*dcos(g1)+dcos(b1)*dsin(g1)*dcos(a1)
		mat(3,1)=-dcos(a1)*dsin(b1)
		
		mat(1,2)=-dcos(a1)*dsin(g1)-dcos(b1)*dcos(g1)*dsin(a1)
		mat(2,2)=dcos(a1)*dcos(g1)-dcos(b1)*dsin(g1)*dsin(a1)
		mat(3,2)=dsin(a1)*dsin(b1)
		
		mat(1,3)=dsin(b1)*dcos(g1)
		mat(2,3)=dsin(b1)*dsin(g1)
		mat(3,3)=dcos(b1)
		endsubroutine
