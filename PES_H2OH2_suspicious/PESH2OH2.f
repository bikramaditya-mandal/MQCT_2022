...
c-------------------------------- PES EVALUATION
      ...
      ENDIF
      icntrl=0
         thx = beta!*180d0/acos(-1d0)
         phx = gamma!*180d0/acos(-1d0)
         thppx = bbeta!*180d0/acos(-1d0)
         phppx = aalpha!*180d0/acos(-1d0)
!         CALL CONVERT_ANGLES(thppx,phppx,r_inp)
!         CALL ROTATE(r_inp,thx,phx,r_out)
!         CALL EXTRACT_ANGLES(thpx,phpx,r_out)
!         thx = thx*180d0/acos(-1d0)
!         phx = phx*180d0/acos(-1d0)
!         thpx = thpx*180d0/acos(-1d0)
!!         phpx = phpx*180d0/acos(-1d0)
! !        IF(phpx.lt.0d0) phpx = 360d0+phpx		 
         call potenl(icntrl, 
...
!            y=y+t(p1,q1,p2,p,thx,phx,thpx,phpx)*coef(i)
            y=y+t_body_fixed(p1,q1,p2,p,thx,phx,thppx,phppx)*coef(i)
 !            IF(p1.eq.2 .and. p.eq.2 .and. p2.eq.0 .and. q1.eq.2 ) THEN			
!            y=y+t_body_fixed(p1,q1,p2,p,thx,phx,thpx,phpx)
!            ENDIF
      ...
      V = y
      CONTAINS 
      SUBROUTINE ROTATE(a_inp,theta,phi,a_out)
      IMPLICIT NONE	  
      REAL*8 a_inp(3), a_out(3)
      REAL*8 a_1(3)
      REAL*8 phi,theta
      a_1(3) = a_inp(3)*cos(theta)  + a_inp(1)*sin(theta)
      a_1(1) = -a_inp(3)*sin(theta) + a_inp(1)*cos(theta)
      a_1(2) = a_inp(2)
      a_out(1) = a_1(1)*cos(phi) + a_1(2)*sin(phi)
      a_out(2) = a_1(2)*cos(phi) - a_1(1)*sin(phi) 
      a_out(3) = a_1(3)		  
      END SUBROUTINE ROTATE
      SUBROUTINE EXTRACT_ANGLES(theta,phi,a_inp)
      IMPLICIT NONE
      REAL*8 theta,phi,a_inp(3),r,rxy
      r = dsqrt(a_inp(1)**2	 + a_inp(2)**2  + a_inp(3)**2) 
      theta = dacos(a_inp(3)/r)	  
      phi = datan2(a_inp(2),a_inp(1))	  
     	  
      END SUBROUTINE EXTRACT_ANGLES
      SUBROUTINE CONVERT_ANGLES(theta,phi,a_out)
      IMPLICIT NONE
      REAL*8 theta,phi,a_out(3),r,rxy
      a_out(3) = cos(theta)
      a_out(2) = sin(theta)*sin(phi)
      a_out(1) = sin(theta)*cos(phi)      	  
	  
     	  
      END SUBROUTINE CONVERT_ANGLES	  	  
      end
      FUNCTION t_body_fixed(p1,q1,p2,p,thx,phx,thpx,phpx)
      IMPLICIT NONE
      REAl*8 thx,phx,thpx,phpx,t_body_fixed
      REAL*8 clb_grd,CG	,wfr ,wfi,dwi1,dwi2,dwr1,dwr2,dwr3,dwi3
	  REAL*8 wfr1,wfr2,wfi1,wfi2
      REAL*8 alpha_factor,delta
      integer p1,q1,p2,p,m
      REAL*8, PARAMETER :: pi = acos(-1d0)
      EXTERNAL Djkm, WF_DIAT_TOP,CG,delta	
      IF(max(p,p1,p2)*2 .gt.p1+p2+p ) THEN
      PRINT*,p,p1,p2
      STOP"ERROR:TRINAGULAR RULE"
      ENDIF	
      alpha_factor = sqrt(2d0*(1d0+delta(q1,0))*(2d0*p1+1))	  
      wfr = 0d0
      wfi = 0d0
      wfr1 = 0d0
      wfi1 = 0d0
      wfr2 = 0d0
      wfi2 = 0d0	  
      DO m=-min(p1,p2),min(p2,p1)
      IF(abs(m).gt.p1) CYCLE	  
      CALL Djkm(dwr1,dwi1,p1,q1,-m,0d0,thx,phx)
      CALL WF_DIAT_TOP(p2,m,thpx,phpx,dwr2,dwi2)
      clb_grd=CG(p1,p2,p,-m,m,0)  
      wfr =wfr +  clb_grd*(dwr1*dwr2 - 
     &	dwi1*dwi2)
      wfi =wfi +  clb_grd*(dwr1*dwi2 + 
     &	dwr2*dwi1)
      ENDDO
	  wfr = wfr*sqrt((2d0*p1+1d0)/4/pi)
	  !!!	  t_body_fixed = wfr // ends here
      DO m=-min(p1,p2),min(p2,p1)
      IF(abs(m).gt.p1) CYCLE	  
      CALL Djkm(dwr1,dwi1,p1,-q1,-m,0d0,thx,phx)
      CALL WF_DIAT_TOP(p2,m,thpx,phpx,dwr2,dwi2)
      clb_grd=CG(p1,p2,p,-m,m,0)  
      wfr1 =wfr1 +  clb_grd*(dwr1*dwr2 - 
     &	dwi1*dwi2)
      wfi1 =wfi1 +  clb_grd*(dwr1*dwi2 + 
     &	dwr2*dwi1)
      ENDDO	  
      wfr1 = wfr1*sqrt((2d0*p1+1d0)/4/pi)
     & *(-1)**(p1+p2+p+q1)
	  wfr2 = (wfr1+wfr)/sqrt(2d0)
      IF(q1.eq.0)	wfr2=wfr2/sqrt(2d0)  
	  t_body_fixed = wfr2*(-1)**(p1+p2)
      END 	FUNCTION t_body_fixed	 	  
...
c     funct(u)=(1-cos(u*Pi))/2 ; f(u)=funct(funct(u))
      ...
      save
      OPEN(UNIT=6,FILE="PES_H2OH2.out")
...
c-------------------------------------------------------------------
      ...
      dir= '/mmfs1/home/4168mandalb/v20_21/PES_H2OH2/H2Ov000_H2v0/'
      corr_9d_flag = .true.
      r12_flag = .true.
c      read(*,POTL)
      Pi=acos(-1.0d0)
